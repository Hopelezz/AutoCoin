...
class Trends(CoinbaseClient):
    ...
    def check_price_trends():
        ...
        for coin, details in Trends.coins_settings.items():
            if details.get('enabled', False):
                previous_price = details.get('previous_price')
                current_price = CoinbaseClient.get_current_price(f"{coin}-USD")
                ...
                if current_price is not None:
                    ...
                    # Calculate SMA
                    details['sma_short_term'] = Trends.calculate_sma(details['prices'], short_term_window)
                    details['sma_long_term'] = Trends.calculate_sma(details['prices'], long_term_window)
                    ...
                    update_coins_settings(Trends.coins_settings)
                else:
